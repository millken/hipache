...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    readFromCache(host, function (rows) {
        ...
        if (backends.length === 0) {
        ...
        }else if (backends.length === 1) { // the host only one
            var backend = url.parse(backends[0]);
            //console.log(backend);
            ...
            this.debug('Proxying one: ' + host + ' -> ' + backend.hostname + ':' + backend.port);            
            return callback(false, 0, backend);
        }
        var deads = rows[3];
        if (backends.length === 0) {
        ...
        //console.log("hostKey:"+host,rows,backends);
        //console.trace();
        var index = (function () {
        ...
        if (backend.hostname == undefined) { //alert(null==undefined);
            return callback('backend is invalid', 502);
        }
        backend.port = (backend.port == undefined) ? 80 : parseInt(backend.port, 10);
