...
Worker.prototype.runServer = function (config) {
    ...
    var proxyResponseHandler = function(req, res, upstreamResponse) {
        // cache to redis
        var cacheKey = "cache:" + req.headers['host'] + req.url + "-" + req.method;
        //console.log({"req":req});
        console.log("cache url key : "+cacheKey);
        ...
        //fs.writeFileSync(rootDir+"/cache.tmp", responseString);
    ...
    var httpRequestHandler = function (req, res) {
        ...
        (function () {
            ...
            res.writeHead = function (statusCode) {
                ...
                if (config.response !== undefined && config.response.replaceHeader !== undefined) {
                    for (var n in config.response.replaceHeader) {
                        res.setHeader(n, config.response.replaceHeader[n]);
                    }
                }
                /*show servername in header*/
        ...
        var proxyRequest = function () {
            ...
            this.cache.getBackendFromHostHeader(req.headers.host, function (err, code, backend) {
                if (err) {
                    console.log({"err":err, "code":code, "backend":backend});
                ...
                //proxy.on('proxyResponse', new httpCacher(config).ResponseHandler);
